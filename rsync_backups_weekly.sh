...
export LANG=C.UTF-8
# =============================================================================
#          file: /home/victoria/backups/rsync/rsync_backups_weekly.sh
#         title: HOURLY rsync backup script
#       created: 2020-04-29
#       version: 02
# last modified: 2020-05-12 08:12:16 -0700 (PST)
#      versions:
#         * v01: inaugural
#         * v02: 
...
printf "`date +'%Y-%m-%d (%H:%M:%S)'`: START WEEKLY\n"

# INCREMENT:
printf "`date +'%Y-%m-%d (%H:%M:%S)'`: cp weekly.x --> weekly.x+1\n"
rm -fr /mnt/Backups/rsync_backups/weekly.3
mv 2>/dev/null  /mnt/Backups/rsync_backups/weekly.2/ /mnt/Backups/rsync_backups/weekly.3
mv 2>/dev/null  /mnt/Backups/rsync_backups/weekly.1/ /mnt/Backups/rsync_backups/weekly.2
mv 2>/dev/null  /mnt/Backups/rsync_backups/weekly.0/ /mnt/Backups/rsync_backups/weekly.1

# COPY:
printf "`date +'%Y-%m-%d (%H:%M:%S)'`: cp daily.6 --> weekly.0\n"
# https://stackoverflow.com/questions/1529946/linux-copy-and-create-destination-dir-if-it-does-not-exist
cp 2>/dev/null -al /mnt/Backups/rsync_backups/daily.6/ /mnt/Backups/rsync_backups/weekly.0

printf "`date +'%Y-%m-%d (%H:%M:%S)'`: END WEEKLY\n"
